...
多显示器时，面板会**按真实排列铺开所有屏幕**，一个手势即可选「任意屏 × 任意区域」。停在**别的屏幕那张卡片的中间格子**上，窗口会原封不动地搬到那块屏幕——大小不变、位置不变，而不是被平铺；格子里画的小方块就是它落点的预览。当前这块屏幕卡片的中间格子仍然是取消。还有一个「仅拖动触发」模式，让普通的中键单击保持完全原生——只有真正移动光标后面板才出现。
